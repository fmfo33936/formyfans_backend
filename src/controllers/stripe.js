...
async function handleCreatorInvoicePaid(invoice) {
  ...
  console.log(
    `Creator subscription payment: fanId=${fanSub.fanId}, creatorId=${fanSub.creatorId}, amount=${invoice.amount_paid}`,
  );
  ...
  console.log(
    `Fan payment history created: fanId=${fanSub.fanId}, amount=${centsToDollars(invoice.amount_paid)}`,
  );
}
