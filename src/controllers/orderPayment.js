...
const { getStripe, dollarsToCents, centsToDollars } = require("../utils/stripe");
...
const applyPaymentIntentToOrder = async (order, paymentIntent, { sendEmails = false }) => {
...
const retrieveAndValidatePaymentIntent = async (paymentIntentId, order) => {
  ...
  if (
    ...
    return { error: { status: 400, message: "Payment intent does not match this order" } };
  }
...
const createClientSecret = async (req, res) => {
  try {
    ...
    if (!orderId) {
      return res.status(400).json({ message: "orderId is required (e.g. ORD-000001)" });
    }
...
const confirmPayment = async (req, res) => {
  try {
    ...
    const result = await applyPaymentIntentToOrder(order, paymentIntent, { sendEmails: false });
    if (result.error) {
...
const updatePaymentStatusFromStripe = async (req, res) => {
  try {
    ...
    const result = await applyPaymentIntentToOrder(order, paymentIntent, { sendEmails: true });
    if (result.error) {
      return res.status(400).json({
        message: result.error,
        paymentStatus: result.paymentStatus,
